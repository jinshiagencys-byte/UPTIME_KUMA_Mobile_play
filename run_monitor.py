...
Needle 3 (cactus-needle) : extraction structuree locale en secours pour le rapport final
quand le JSON brut de done() est invalide, et pour structurer chaque etape suspecte en
step_details (pre-filtre anomalie : Needle n'est appele que sur les cellules avec erreur
ou indice d'anomalie dans la sortie, pas systematiquement sur toutes les etapes).
"""
import asyncio
import glob
import inspect
...
# ---------------------------------------------------------------------------
# Needle 3 (cactus-needle) — extraction structuree de secours, en local, sans
# cle API. Utilise (1) en secours dans parse_report() quand le JSON brut du
# rapport final est invalide, (2) pour structurer chaque etape suspecte (code+
# sortie) en step_details, expose separement de "pages" pour ne rien changer au
# payload envoye a pages-report.
# ---------------------------------------------------------------------------
try:
    ...
    class StepObservation(BaseModel):
        ...
        anomaly_observed: bool
        anomaly_note: Optional[str] = None

    NEEDLE_AVAILABLE = True
except Exception as _needle_import_err:
    NEEDLE_AVAILABLE = False
    print("Needle 3 indisponible (%s) : extraction structuree de secours desactivee"
          % str(_needle_import_err)[:200])


def needle_extract_report(text):
    """Secours : extrait un rapport final structure via Needle 3 quand le JSON
    brut est invalide ou absent. Retourne un dict ou None."""
    if not NEEDLE_AVAILABLE or not isinstance(text, str) or not text.strip():
        return None
    try:
        model = _needle.extract(text, FinalReportModel)
        return model.model_dump()
    except Exception as e:
        print("Extraction Needle 3 du rapport final : echec (%s)" % str(e)[:150])
        return None


# ---------------------------------------------------------------------------
# Pre-filtre anomalie : evite un appel Needle 3 quand rien de suspect n'est
# visible dans la sortie/l'erreur de la cellule (economie d'inference locale
# sur la majorite des etapes, qui n'ont aucune anomalie).
# ---------------------------------------------------------------------------
_ANOMALY_HINT_RE = re.compile(
    r"error|exception|traceback|undefined|null|introuvable|failed|timeout"
    r"|erreur|echec|manquant",
    re.IGNORECASE,
)


def structure_step_observations(cells, model_name):
    """Structure via Needle 3 chaque etape reellement executee (code + sortie
    ou erreur) en {action_description, anomaly_observed, anomaly_note}.
    N'appelle Needle que si la sortie/erreur contient un indice d'anomalie ;
    sinon, observation neutre construite localement (pas d'inference).
    Degrade silencieusement (liste vide) si Needle est indisponible."""
    if not NEEDLE_AVAILABLE:
        return []
    details = []
    for idx, cell in enumerate(cells or []):
        source = (getattr(cell, "source", "") or "").strip()
        if not source or "done(" in source:
            continue
        output = (getattr(cell, "output", "") or "").strip()
        error = getattr(cell, "error", None)
        raw = str(error) if error else output

        if not (error or _ANOMALY_HINT_RE.search(raw)):
            details.append({
                "step": idx + 1,
                "action_description": source[:150],
        ...
        text = "Code execute (%s) :\n%s\n\nResultat :\n%s" % (
            model_name, source[:800], raw[:800])
        try:
            obs = _needle.extract(text, StepObservation)
            details.append({"step": idx + 1, **obs.model_dump()})
        except Exception as e:
            print("Extraction Needle 3 etape %d : echec (%s)" % (idx + 1, str(e)[:120]))
    return details
...
def parse_report(text):
    ...
    if (
        ...
        return candidate
    # Secours : le JSON brut est invalide/absent (ex. SyntaxError sur un
    # guillemet typographique) -> extraction structuree via Needle 3
    needle_candidate = needle_extract_report(text if isinstance(text, str) else None)
    if (
        isinstance(needle_candidate, dict)
        and str(needle_candidate.get("overall_status", "")).upper() in VALID_STATUSES
    ):
        print("Rapport final recupere via extraction structuree Needle 3 (JSON brut invalide)")
        return needle_candidate
    return None
...
def normalize_report(report, model_name, target_url):
    ...
    # Coherence dans les deux sens entre le verdict global et les assertions par page :
    # un rapport ne peut pas etre UP si une assertion a echoue, et ne devrait pas
    # rester DOWN si TOUTES les assertions ont reussi (incoherence deja observee
    # dans des runs reels : l'agent ecrit overall_status=DOWN alors que pages[]
    # ne contient que des succes).
    failed = [p for p in pages if _is_false(p.get("assertion_passed"))]
    passed_all = bool(pages) and all(_as_bool(p.get("assertion_passed")) for p in pages)
    ...
    elif status == "DOWN" and passed_all:
        status = "UP"
        print("Verdict corrige DOWN -> UP : toutes les assertions ont reussi (incoherence de l'agent)")
...
async def run_attempt(model_config, target_url, requirements, deadline):
    ...
    outcome = {"report": None, "partial": (None, 0), "stop": False, "step_details": []}
    agent = None
    ...
    try:
        ...
        try:
            ...
            outcome["step_details"] = structure_step_observations(cells, model_name)
...
async def test_page(target_url, requirements, deadline):
    ...
    for idx, model_config in enumerate(MODEL_CHAIN):
        ...
        if outcome["report"] is not None:
            return outcome["report"], best_partial, False, outcome.get("step_details", [])
        if outcome["partial"][1] > best_partial[1]:
            best_partial = outcome["partial"]
            best_step_details = outcome.get("step_details", [])
        if outcome["stop"]:
    ...
    return None, best_partial, all_dead, best_step_details
...
async def main():
    ...
    for i, url in enumerate(targets):
        ...
        report, partial, stop, step_details = await test_page(url, requirements, deadline)
        stopped = stopped or stop
        if step_details:
            step_details_by_url[url] = step_details
    ...
    final_report = {
        ...
        "pages": page_entries,
        # Cle separee de "pages" (pages-report ne lit que .pages, donc ceci ne
        # part jamais vers le relay) : observations structurees par etape,
        # extraites via Needle 3, une par page testee.
        "step_details": step_details_by_url,
    }
